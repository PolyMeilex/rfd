...
- It is also recomended to always spawn dialogs on main thread, RFD will do it's best to run dialogs from any thread but it is only posible in windowed app and it is still WIP (only async dialogs can do it as of now). For now it is recomended to: [spawn on main and await in other thread](https://github.com/PolyMeilex/rfd/blob/master/examples/async.rs)
